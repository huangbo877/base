...
import com.xpzones.base.SimpleBaseModel;
import com.xpzones.utils.Convert;
...
//        LogUtil.Log("rawType:" + rawType.toString());
        ...
        if (typeArgument == Void.class) {
            ...
            SimpleBaseModel baseModel = Convert.fromJson(jsonReader, BaseModel.class);
            response.close();
            //noinspection unchecked
            return (T) baseModel.toBaseModel();
        } else if (rawType == BaseModel.class) {
            //有数据类型，表示有data
            BaseModel lzyResponse = Convert.fromJson(jsonReader, type);
            response.close();
            ...
            if (status == 1) {
                ...
                throw new IllegalStateException(lzyResponse.msg);//该抛出错误，会在onError中回调
            }
        } else {
            response.close();
